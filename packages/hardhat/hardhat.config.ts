...
task(
  ...
  async (_, { ethers }) => {
    ...
    Object.keys(config.networks).forEach(async (networkName) => {
      // @ts-ignore
...
function send(signer: JsonRpcSigner, txparams: Record<string, any>) {
...
task("send", "Send ETH")
  ...
  .setAction(async (taskArgs, { network, ethers }) => {
    ...
    debug(taskArgs.gasPrice / 1000000000 + " gwei");
